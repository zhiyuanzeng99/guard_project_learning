# OpenGuardrails 云服务器实践项目指南

> **硬件环境**: RTX 4090 GPU (24GB) + CUDA 12.8 + Python 3.12 (Ubuntu 22.04)
>
> **云服务器配置**: 16 vCPU Intel Xeon + 120GB 内存 + PyTorch 2.8.0
>
...
**AutoDL已预装所有基础依赖，以下命令可直接执行：**

```bash
# 1. 安装Ollama（AutoDL网络正常，可直接安装）
curl -fsSL https://ollama.com/install.sh | sh

# 2. 启动Ollama服务（后台运行）
nohup ollama serve > /dev/null 2>&1 &

# 3. 拉取模型（RTX 4090 24GB可运行标准版）
ollama pull llama3.1          # Agent使用的LLM
ollama pull nomic-embed-text  # OpenGuardrails使用的embedding模型
...
> 📌 **环境说明**: 云服务器已预装Python 3.12和PyTorch，可直接使用系统Python或创建venv

### 关键决策点 (全部已优化)

| 决策点 | 选项 | ✅ 推荐 | 原因 |
|--------|------|--------|------|
| **部署方式** | Docker / 手动部署 | **先测试Docker** | 云服务器通常Docker可用，如不行再手动部署 |
| **LLM选择** | Ollama / vLLM / OpenAI | **Ollama llama3.1** | 安装简单，性能足够，5分钟启动 |
| **检测模型** | Ollama / vLLM官方模型 | **Ollama (先)** | 快速验证，后期可升级 |
| **Workers数量** | 4-32 | **16 (检测) + 12 (代理)** | 已针对RTX 4090优化 |
| **测试规模** | 快速/完整 | **快速模式 (15用例)** | 首次验证15分钟，完整可后补 |
| **升级vLLM** | 现在/稍后 | **完成基础测试后** | 避免复杂度，按需升级 |

### 推荐时间线

```
总时间: 4-5小时 (云服务器已有Python/PyTorch，更快！)

1. 环境准备 (20分钟) ⬇️ 云服务器已有基础环境
   └─ 测试Docker + 安装Ollama

2. 部署Agent (30分钟) ⬇️ 云服务器性能更强
   └─ 使用Ollama llama3.1标准版

3. 部署OpenGuardrails (1小时)
   └─ Docker方式 或 手动部署
...
```bash
# 这是你需要的全部配置
- 环境: python3 -m venv venv && source venv/bin/activate (云服务器已有Python)
- 模型: ollama pull llama3.1 + ollama pull nomic-embed-text
- 配置: 使用文档中【推荐配置 - RTX 4090优化版】
...
**✅ 云服务器优势：Python/PyTorch/CUDA已经预装好，省去大量配置时间！**

### 1.2 测试Docker可用性（重要！）

**云服务器通常Docker网络正常，先测试一下：**
...
| 测试结果 | 推荐方案 |
|----------|----------|
| ✅ `docker pull` 成功 | 使用Docker部署（简单快速） |
| ❌ `docker pull` 失败 | 使用手动部署（见3.8节） |
...
source ~/openguardrails-practice/venv/bin/activate  # 激活虚拟环境
...
# ============================================
# 服务Workers配置 - ✅ RTX 4090优化配置
# ============================================
# 根据RTX 4090的性能，适度增加workers
...
- ⚠️ Workers配置: 16/12已针对RTX 4090优化，无需调整
...
### 3.8 (备选) 手动部署OpenGuardrails（不使用Docker）

**⚠️ 只有在Docker无法使用时才选择此方案**

如果云服务器Docker网络不通，可以手动部署：
...
# 如果都返回 {"status":"healthy"}，说明部署成功！
```

**注意：** 手动部署不包含前端界面，需要通过API操作或自行构建前端。
...
验证OpenGuardrails对Agent的安全防护效果，对��有无OpenGuardrails保护的差异。
...
# 测试Docker是否可用
docker --version
docker pull hello-world  # 如果成功，使用Docker方案；否则使用手动部署
